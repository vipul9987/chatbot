        ...
        <div className="pointer-events-auto mb-4 w-full max-w-[400px] h-[min(600px,calc(100vh-8rem))] bg-white/95 backdrop-blur-xl rounded-[2.5rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.15)] border border-zinc-200/50 flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500 ease-[cubic-bezier(0.2,0.8,0.2,1)]">
          ...
          <div className="px-8 py-6 bg-zinc-950 text-white flex items-center justify-between relative overflow-hidden shrink-0">
            ...
            <div className="flex items-center gap-4 relative z-10">
              <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center p-0.5 shadow-lg">
                <div className="w-full h-full bg-zinc-900 rounded-[0.5rem] flex items-center justify-center font-bold text-white text-[10px] tracking-tighter">
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[8px] text-zinc-400 font-bold uppercase tracking-[0.1em]">Online</span>
                </div>
              </div>
            </div>
          ...
          {/* Messages Area */}
          <div className="flex-1 overflow-y-auto p-6 space-y-2 bg-zinc-50/20">
            {messages.map((msg) => (
              <ChatBubble key={msg.id} message={msg} />
            ))}
            {chatState === ChatState.TYPING && (
              <div className="flex justify-start mb-4">
                <div className="bg-white/80 px-4 py-3 rounded-[1.2rem] rounded-tl-none border border-zinc-100 shadow-sm flex gap-1 items-center">
          ...
          <div className="p-5 bg-white border-t border-zinc-100 shrink-0">
            <form onSubmit={handleSend} className="relative flex items-center">
              <input
                ...
                className="w-full pl-5 pr-12 py-3.5 bg-zinc-50/50 border border-zinc-100 rounded-2xl text-[14px] font-medium text-zinc-900 placeholder:text-zinc-400 focus:outline-none focus:bg-white focus:border-zinc-300 transition-all"
              />
              <button
                type="submit"
                disabled={!inputValue.trim() || chatState === ChatState.TYPING}
                className="absolute right-1.5 w-9 h-9 bg-zinc-950 text-white rounded-[0.9rem] flex items-center justify-center hover:bg-zinc-800 disabled:opacity-20 transition-all active:scale-95 shadow-md"
              >
      ...
      >
        {isOpen ? (
          ...
          <div className="relative">
            ...
            <div className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-white rounded-full border-2 border-black" />
          </div>
        )}
      </button>
    </div>
