    ...
    <div className="fixed inset-0 flex flex-col items-end justify-end p-6 pointer-events-none">
      ...
      {isOpen && (
        <div className="interactive mb-6 w-full max-w-[400px] h-[640px] max-h-[85vh] glass-panel rounded-[2.5rem] shadow-[0_40px_120px_-20px_rgba(0,0,0,0.3)] border border-white/50 flex flex-col overflow-hidden animate-widget-in origin-bottom-right">
          ...
          {/* Elegant Dark Header */}
          <div className="px-8 py-7 bg-zinc-950 text-white flex items-center justify-between shrink-0">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-zinc-800 to-zinc-900 border border-white/10 flex items-center justify-center shadow-lg shadow-black/20">
                <span className="font-bold text-lg text-blue-400">{APP_CONFIG.agencyName[0]}</span>
              </div>
              <div>
                <h3 className="text-[16px] font-bold tracking-tight">Expert Assistant</h3>
                <div className="flex items-center gap-2 mt-1">
                  <span className="relative flex h-2 w-2">
                    <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                    <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
                  </span>
                  <span className="text-[10px] font-bold uppercase tracking-[0.1em] text-emerald-400/80">Online & Ready</span>
                </div>
              </div>
            </div>
            <button 
          ...
          {/* Messages Container */}
          <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6 bg-[#fcfcfd]/80">
            {messages.map((msg) => (
          ...
          {/* Modern Input Bar */}
          <div className="p-6 bg-white/50 backdrop-blur-sm border-t border-zinc-100/50">
            <form onSubmit={handleSend} className="relative flex items-center">
              <input
                ...
                className="w-full pl-6 pr-14 py-4.5 bg-zinc-100/60 border border-transparent rounded-[1.25rem] text-[15px] focus:outline-none focus:bg-white focus:border-blue-500/20 focus:ring-4 focus:ring-blue-500/5 transition-all placeholder:text-zinc-400"
              />
              <button
                type="submit"
      ...
      <button
        ...
        className={`interactive relative group w-16 h-16 rounded-full flex items-center justify-center shadow-[0_20px_60px_rgba(0,0,0,0.3)] transition-all duration-500 transform active:scale-90 ${
          isOpen ? 'bg-zinc-950 rotate-180' : 'bg-zinc-950 hover:scale-110'
        }`}
      >
        {/* The Glow Ring - Exactly as requested */}
        <div className={`absolute inset-0 rounded-full border-[3px] transition-all duration-500 ${
          isOpen 
            ? 'border-zinc-800 scale-95 opacity-50' 
            : 'border-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.5)] opacity-100'
        }`}></div>
